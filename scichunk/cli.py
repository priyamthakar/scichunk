...
@click.option("--include-metadata/--no-metadata", default=None, help="Include rich chunk metadata in exported output.")
...
def process(
    ...
    include_metadata = config.include_metadata if include_metadata is None else include_metadata
    ...
    for file_path in files:
        ...
        if output_format == "json":
            destination.write_text(
                json.dumps([chunk.to_dict(include_metadata=include_metadata) for chunk in chunks], indent=2, ensure_ascii=False),
                encoding="utf-8",
            )
        elif output_format == "markdown":
            destination.write_text(_to_markdown(chunks, include_metadata=include_metadata), encoding="utf-8")
        else:
            destination.write_text(_to_txt(chunks, include_metadata=include_metadata), encoding="utf-8")
...
def _to_markdown(chunks, include_metadata: bool = True) -> str:
    parts: list[str] = []
    for chunk in chunks:
...
def _to_txt(chunks, include_metadata: bool = True) -> str:
    parts: list[str] = []
    for chunk in chunks:
        header = f"[{chunk.id}] {chunk.section}"
        if include_metadata:
            header += f" | {chunk.token_count} approx. tokens"
        parts.append(header)
